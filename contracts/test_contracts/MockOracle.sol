...
contract MockOracle is ChainlinkRequestInterface, LinkTokenReceiver {
  using SafeMathChainlink for uint256;
    ...
    // calldata is a special data location that contains 
    // the function arguments, only available for external 
    // function call parameters
    // Calldata is a non-modifiable, non-persistent area 
    ...
    checkCallbackAddress(_callbackAddress)
  {
    ...
    modifier isValidRequest(bytes32 _requestId) {
        require(commitments[_requestId].callbackAddr != address(0), "Must have avalid requestId");
        _;
    }
